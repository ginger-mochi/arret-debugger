...
external processes to send commands one at a time, like GDB.

Each connection is one-shot: connect, send one line, read one JSON response line,
connection closes. This makes it easy to use from shell scripts or tools.
...
These commands control the frontend's display, audio, and input handling at runtime.

- **`manual on|off`** - Enable or disable keyboard input processing. Defaults to `on`
  in headed mode, `off` in headless mode. Does not affect TCP command input.
- **`display on|off`** - Open or close the SDL display window. Works from headless
  mode too (lazily initializes SDL). Closing the display does not affect `manual` state.
- **`sound on|off`** - Enable or disable audio output. Lazily initializes SDL audio
  if needed. The `--mute` flag is equivalent to starting with `sound off`.

## Headed Mode Keyboard

Keyboard input is only processed when `manual` is `on` (default in headed mode).
...
| Return | Start |
| Right Shift | Select |
| Shift+1-9, Shift+0 | Save to slot 1-9, 0 |
| Ctrl+1-9, Ctrl+0 | Load from slot 1-9, 0 |
